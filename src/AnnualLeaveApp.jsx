    ...
    return (
        <AppContext.Provider value={ctx}>
            <div className="min-h-screen bg-slate-50 font-sans text-slate-800 print:bg-white print:min-h-0">
                ...
                {/* 인쇄 시 앱 UI 숨김 처리 (`print:hidden`)로 2페이지 백지 원천 차단 */}
        ...
        <div className="flex items-center justify-center min-h-screen p-4">
            <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-md relative">
                    ...
                    {mode === 'user' && (
                        <>
                            ...
                            <div>
                                ...
                                <input required type="text" value={name} onChange={e => setName(e.target.value)} autoComplete="off" spellCheck={false} className="w-full p-3 border rounded bg-white focus:border-indigo-500 outline-none" placeholder="본명 입력" />
                            </div>
                        </>
                    )}
                    <div>
                        ...
                        <input required type="password" value={pw} onChange={e => setPw(e.target.value)} autoComplete="off" spellCheck={false} className="w-full p-3 border rounded bg-white focus:border-indigo-500 outline-none" />
                    </div>
                ...
                <div className="absolute bottom-6 right-6">
                    <button type="button" onClick={() => { setMode(mode === 'admin' ? 'user' : 'admin'); setName(''); setPw(''); setDept(departments[0]||''); }} className="text-xs text-slate-300 hover:text-slate-500 font-medium transition-colors">{mode === 'admin' ? '직원 접속' : '관리자 접속'}</button>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow border">
            ...
            <div className="space-y-4 text-sm">
                ...
                <div><label className="block font-bold mb-1">사유/적요</label><input type="text" value={applyRemark} onChange={e => setApplyRemark(e.target.value)} autoComplete="off" spellCheck={false} className="w-full border p-3 rounded focus:border-indigo-500 outline-none bg-slate-50" placeholder="개인사정 등" /></div>
    ...
    return (
        <div className="w-full lg:w-1/3 bg-slate-50 p-4 md:p-6 rounded border overflow-auto shrink-0 max-h-96 lg:max-h-none">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-sm">
                ...
                <div><label className="block font-bold mb-1">성명</label><input required type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} autoComplete="off" spellCheck={false} className="w-full border p-2 rounded" placeholder="본명 입력 (저장시 암호화)" /></div>
...
const PrintApplicationModal = ({ record, user, approvalLine, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-200/60 flex items-center justify-center z-[100] p-4 print:static print:block print:bg-white print:p-0">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] print:max-w-none print:w-full print:shadow-none print:max-h-none print:border-none print:block print:overflow-visible">
            ...
            <style dangerouslySetInnerHTML={{ __html: `
                @media print {
                    ...
                    #print-area { position: absolute; left: 0; top: 0; width: 100%; box-sizing: border-box; background: white; }
                }
            `}} />
        </div>
    );
...
const PrintSummaryModal = ({ employee, records, gen, used, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-200/60 flex items-center justify-center z-[100] p-4 print:static print:block print:bg-white print:p-0">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] print:max-w-none print:w-full print:shadow-none print:max-h-none print:border-none print:block print:overflow-visible">
                <div className="p-4 bg-slate-100 border-b flex justify-between items-center print:hidden shrink-0">
...
const PrintPromotionModal = ({ allEmployees, records, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-200/60 flex items-center justify-center z-[100] p-4 print:static print:block print:bg-white print:p-0">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[95vh] print:max-w-none print:w-full print:shadow-none print:max-h-none print:border-none print:block print:overflow-visible">
                <div className="p-4 bg-slate-100 border-b flex justify-between items-center print:hidden shrink-0">
