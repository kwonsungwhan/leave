...
export default function AnnualLeaveApp() {
    ...
    useEffect(() => {
        ...
        const unsubSettings = onSnapshot(doc(db, publicPath, 'settings', 'global'), (snap) => {
            if (snap.exists()) {
                ...
                setDoc(doc(db, publicPath, 'settings', 'global'), { departments: ['관리소', '경비반', '미화반'], adminPassword: '1234', approvalLine: ['담 당', '과 장', '소 장'], companyName: '우리회사(주)' });
            }
        });
            ...
            for (const emp of employees) {
                ...
                for (let m = 1; m <= 11; m++) {
                    ...
                    if (today >= new Date(targetDateStr)) {
                        ...
                        try { await setDoc(doc(db, publicPath, 'leaveRecords', uniqueId), { date: targetDateStr, type: '발생', days: 1, remark: `입사 ${m}개월 만근 연차 (시스템 자동 발생)`, isAuto: true, isFulfilled: true, empId: emp.empId, dept: emp.dept, name: emp.name, realName: emp.realName, isCanceled: false }, { merge: true }); } catch (e) {}
                    }
                }
                ...
                if (years >= 1) {
                    for (let y = 1; y <= Math.floor(years); y++) {
                        ...
                        if (today >= new Date(targetDateStr)) {
                            ...
                            const uniqueId = `auto-${emp.empId}-${targetDateStr}`;
                            try { await setDoc(doc(db, publicPath, 'leaveRecords', uniqueId), { date: targetDateStr, type: '발생', days: base, remark: `입사 ${y}년차 연차 (시스템 자동 발생)`, isAuto: true, isFulfilled: true, empId: emp.empId, dept: emp.dept, name: emp.name, realName: emp.realName, isCanceled: false }, { merge: true }); } catch (e) {}
                        }
                    }
                }
            }
    ...
    const ctx = useMemo(() => ({ user, setUser, employees, departments, leaveRecords, adminPassword, approvalLine, companyName, showToast, showConfirm, dbUpdateSettings, publicPath }), [user, employees, departments, leaveRecords, adminPassword, approvalLine, companyName, publicPath]);
    ...
    return (
        <AppContext.Provider value={ctx}>
            ...
            `}} />
            <div className="min-h-screen bg-slate-50 font-sans text-slate-800 h-full">
                ...
                {/* 
                  라우터 렌더링 영역 (인쇄 시 숨김 클래스인 print:hidden은 
                  이 루트 레벨이 아니라 각 AdminView, UserView 내부의 UI 박스에만 적용해야 합니다.
                  그래야 모달이 인쇄될 때 투명인간 취급을 받지 않습니다.)
                */}
...
// --------------------------------------------------------
// 독립 폼 컴포넌트 (모바일 키보드 씹힘 완벽 차단 - useRef 비제어 방식)
// --------------------------------------------------------
function LoginView() {
    ...
    // 키보드 씹힘/렉 방지를 위한 비제어(uncontrolled) Ref 사용
    ...
    const handleLogin = (e) => {
        ...
        const inputPw = pwRef.current ? pwRef.current.value : '';
                    ...
                    {mode === 'user' && (
                        <>
                            ...
                            <div>
                                ...
                                {/* 모바일 한글 씹힘 방지: Ref 사용 및 자동완성 속성 제거 */}
                                <input required ref={nameRef} type="text" defaultValue="" className="w-full p-3 border rounded bg-white focus:border-indigo-500 outline-none" placeholder="본명 입력" />
                            </div>
                        </>
                    )}
                    <div>
                        ...
                        <input required ref={pwRef} type="password" defaultValue="" className="w-full p-3 border rounded bg-white focus:border-indigo-500 outline-none" />
                    </div>
...
const UserApplyForm = ({ onSubmit }) => {
    ...
    // 모바일 한글 씹힘 방지를 위한 비제어(uncontrolled) Ref 사용
    ...
    const handleSubmit = (e) => {
        ...
        onSubmit({ 
            applyDate, 
            applyDays, 
            applyRemark: remarkRef.current ? remarkRef.current.value : '' 
        });
        setApplyDate(''); 
        setApplyDays(1); 
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow border shrink-0">
            ...
            <div className="space-y-4 text-sm">
                ...
                <div><label className="block font-bold mb-1">사유/적요</label><input ref={remarkRef} type="text" defaultValue="" className="w-full border p-3 rounded focus:border-indigo-500 outline-none bg-slate-50" placeholder="개인사정 등" /></div>
    ...
    return (
        <div className="w-full lg:w-1/3 bg-slate-50 p-4 md:p-6 rounded border overflow-auto shrink-0 max-h-96 lg:max-h-none">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-sm">
                <div><label className="block font-bold mb-1">사원번호</label><input required disabled={!!editingEmp} type="text" value={form.empId} onChange={e => setForm({...form, empId: e.target.value})} className="w-full border p-2 rounded disabled:bg-slate-200" placeholder="예: 2026001" /></div>
                ...
                <div><label className="block font-bold mb-1">성명</label><input required type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full border p-2 rounded" placeholder="본명 입력 (저장시 암호화)" /></div>
                {!editingEmp && <div><label className="block font-bold mb-1">초기 비밀번호</label><input type="text" value={form.pw} onChange={e => setForm({...form, pw: e.target.value})} className="w-full border p-2 rounded" /></div>}
                ...
                <div><label className="block font-bold mb-1">비고</label><input type="text" value={form.remark} onChange={e => setForm({...form, remark: e.target.value})} className="w-full border p-2 rounded" /></div>
                <div className="flex gap-2 pt-2">
                    {editingEmp && <button type="button" onClick={onCancel} className="flex-1 bg-slate-300 text-slate-700 py-2.5 rounded font-bold hover:bg-slate-400">취소</button>}
                    <button type="submit" className="flex-1 bg-indigo-600 text-white py-2.5 rounded font-bold hover:bg-indigo-700">{editingEmp ? '수정하기' : '등록하기'}</button>
                </div>
            </form>
        </div>
    );
...
const AdminProxyLeaveForm = ({ employees, onSubmit }) => {
    ...
    return (
        <div className="flex items-center gap-3 text-sm border-b pb-3 border-slate-200 flex-nowrap overflow-x-auto whitespace-nowrap">
            ...
            <input type="text" className="border p-1.5 rounded w-32 md:w-48 bg-white" placeholder="사유 (대리신청)" value={form.remark} onChange={e=>setForm({...form, remark:e.target.value})}/>
        ...
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-[100] p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 animate-in zoom-in-95 duration-200">
                ...
                <div className="space-y-4">
                    ...
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">적요(사유/비고)</label><input type="text" value={editData.remark || ''} onChange={e=>setEditData({...editData, remark:e.target.value})} className="w-full border p-2 rounded bg-slate-50"/></div>
                    {editData.isAuto && (
...
const PrintApplicationModal = ({ record, user, approvalLine, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-200/60 flex items-center justify-center z-[100] p-4 print:static print:block print:bg-transparent print:p-0 print:z-auto">
...
function AdminView() {
    ...
    const handleProxySubmit = async (form) => {
        ...
        try {
            await setDoc(doc(db, publicPath, 'leaveRecords', newId), {
                ...
                date: form.date, type: '사용', days: parseFloat(form.days), remark: form.remark, isCanceled: false,
    ...
    return (
        <div className="max-w-[1200px] mx-auto p-2 md:p-4 min-h-screen md:h-screen flex flex-col">
            {/* 
              모달창들은 숨김 처리(print:hidden)되는 앱 UI 바깥에 위치해야 
              인쇄할 때 투명인간이 되지 않고 정상적으로 출력됩니다.
            */}
                    ...
                    {tab === '휴가 내역(전체)' && (
                        <div className="flex flex-col h-full gap-4 overflow-hidden">
                            <div className="bg-slate-50 p-4 border rounded flex flex-col gap-4 shrink-0 overflow-x-auto">
                                ...
                                <AdminProxyLeaveForm employees={employees} onSubmit={handleProxySubmit} />
...
function UserView() {
    ...
    const handleApply = async (form) => {
        ...
        try {
            await setDoc(doc(db, publicPath, 'leaveRecords', newId), {
                ...
                date: form.applyDate, type: '사용', days: parseFloat(form.applyDays), remark: form.applyRemark, isCanceled: false,
    ...
    return (
        <div className="max-w-[1000px] mx-auto p-2 md:p-4 min-h-screen md:h-screen flex flex-col">
            {/* 모달은 인쇄 숨김 바깥에 위치해야 합니다 */}
